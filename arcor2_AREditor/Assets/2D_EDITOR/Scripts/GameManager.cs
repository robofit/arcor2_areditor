...
public class GameManager : Base.Singleton<GameManager>
{
    ...
    string LoadedScene;
    JSONObject ProjectJSON, SceneJSON;
    bool SceneReady;

    public enum ConnectionStatusEnum { Connected, Disconnected }

    ConnectionStatusEnum _ConnectionStatus;

    public ConnectionStatusEnum ConnectionStatus { get => _ConnectionStatus; set
        {
    ...
    void OnConnectionStatusChanged(ConnectionStatusEnum newState)
    {
        switch (newState)
        {
    ...
    string GetFreeIOName(string ioType)
    {
        int i = 1;
        bool hasFreeName;
