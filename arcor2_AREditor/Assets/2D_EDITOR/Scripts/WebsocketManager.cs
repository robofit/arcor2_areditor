...
public class WebsocketManager : Base.Singleton<WebsocketManager> {
    ...
    List<string> ActionObjectsToBeUpdated = new List<string>();
    Queue<string> SendingQueue = new Queue<string>();
    string WaitngForObjectActions;

    bool WaitingForMessage = false;

    string ReceivedData;

    bool ReadyToSend, IgnoreProjectChanged, Connecting;
    ...
    async void Update() {
        ...
        if (WaitngForObjectActions == "" && ActionObjectsToBeUpdated.Count > 0) {
            WaitngForObjectActions = ActionObjectsToBeUpdated[0];
            ActionObjectsToBeUpdated.RemoveAt(0);
            UpdateObjectActions(WaitngForObjectActions);
        }
